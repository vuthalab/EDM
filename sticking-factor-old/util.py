...
rc('font',**{'family':'sans-serif','sans-serif':['Linux Biolinum O']}) # alternative: 'Linux Biolinum O'
...
def variance_breakdown(var):
    total = var.s
    for (component, contribution) in sorted(var.error_components().items(), key=lambda x: -x[1]):
        fraction = 100 * np.square(contribution/total)
        if fraction < 5e-4: continue
        print(f'  {Fore.YELLOW}{component.tag:15s}{Style.RESET_ALL} | {fraction:7.3f} {Style.DIM}%{Style.RESET_ALL}')
...
def odr_fit(model, x, y, p0, absolute_sigma=False):
...
def interp(x, xp, fp):
    return uarray(
        np.interp(nom(x), nom(xp), nom(fp)),
        np.interp(nom(x), nom(xp), std(fp))
    )

...
def melt_and_anneal(end_temp = 7):
    melt_crystal(end_temp = 9.8)

    # Anneal.
    log_entry('Annealing (starting neon line).')
#    mfc.flow_rate_cell = 0.5
    mfc.flow_rate_cell = 1
    T1.ramp_temperature('heat saph', 9, 0.005)
    wait_until_quantity(('temperatures', 'saph'), '<', 9.1, unit='K', source='ctc')
...
#        start_temp = 6.5,
        start_temp = 5,
        neon_flow = 0, buffer_flow = 20,
        growth_time = 10 * MINUTE,
#        anneal=False
    )
#    melt_and_grow(start_temp = 6.5, neon_flow = 14, buffer_flow = 0, growth_time = 20 * MINUTE)

#    grow_only(start_temp=6.5, neon_flow=0, buffer_flow=20, growth_time=15 * MINUTE)
